    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-gray-950/90 backdrop-blur border-b border-gray-800">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6 text-sm text-gray-300">
          ...
          {user && role === 'organizer' && (
            <Link href="/hush/post" className="bg-primary hover:bg-primary-600 px-4 py-2 rounded-lg transition text-white font-medium">
              Post Event
            </Link>
          )}
          ...
          {user && (
            <>
              <Link href="/profile" className="hover:text-accent transition">Profile</Link>
              <button
                onClick={() => signOut()}
                className="hover:text-destructive transition text-gray-400 text-xs"
              >
                Sign out
              </button>
            </>
